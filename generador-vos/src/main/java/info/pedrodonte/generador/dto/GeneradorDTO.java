...
public class GeneradorDTO {
	...
	public GeneradorDTO() {
		super();
		prefijosEntidadesNegocio = (List<String>) ProcesaPropiedades.getInstancia().obtenerConfiguracion().get(PropiedadesManager.PREFIJO_ENTIDA);
		listaNegraatributos = (List<String>) ProcesaPropiedades.getInstancia().obtenerConfiguracion().get(PropiedadesManager.LIST_NEGRA_ATR);
		
		paqueteEntidades = (String) ProcesaPropiedades.getInstancia().obtenerConfiguracion().get(PropiedadesManager.PAQUETE_ORIGEN);
		paqueteCodigoGenerado = (String) ProcesaPropiedades.getInstancia().obtenerConfiguracion().get(PropiedadesManager.PAQUETE_DESTIN);
	}

	@SuppressWarnings("rawtypes")
	public void generar() {
		
		System.out.println(isGenerarVOS+"-"+isGenerarToStringVOS+"-"+isHelperVOBetweenEntity);
		
		contenidoHelperVoEntity.append(NL+"public class "+nombreClaseHelperVoEntity+"{\n"+NL);

		try {
			Class[] lasClases = obtenerClasesDelPaquete(paqueteEntidades);
			
			logger.info(lasClases.length+" Clases encontradas, en "+paqueteEntidades);
			
			for (int i = 0; i < lasClases.length; i++) {
				Class clase = lasClases[i];
				importsClaseHelperVoEntity.append("import "+clase.getName()+";"+NL);
				construirVO(clase);
			}
			...
			if (isHelperVOBetweenEntity) {
				grabarArchivodeHelper(paqueteCodigoGenerado, contenidoHelperVoEntity, getPathArchivosGenerados());
			}
	...
	private void construirVO(Class clase) {
		...
		vos.add(claseVO);
		System.out.println("Generando "+claseVO.getNombreVO()+" "+claseVO.getAtributos().size()+" atributos");
		...
		if (isGenerarVOS) {
			claseVO.grabarArchivodeClaseJava(paqueteCodigoGenerado, getPathArchivosGenerados());
		}
		
		if (isHelperVOBetweenEntity) {
			contenidoHelperVoEntity.append(claseVO.generarContenidoArchivoHelperVO(paqueteCodigoGenerado));
		}
	...
	private String cambioNombreEntityToVO(String nombreAtributo) {
		...
		for (String prefijo : getSufijosNegocio()) {
			if (prefijo.equals("HistoTab")) {
				nombreAtributo = nombreAtributo.replace(prefijo, "VoHisto");
			}else{
	...
	@SuppressWarnings("rawtypes")
	private Class[] obtenerClasesDelPaquete(String packageName)	throws ClassNotFoundException, IOException {
	...
	@SuppressWarnings("rawtypes")
	private List<Class> buscarClasesEnDirectorio(File directory, String packageName) throws ClassNotFoundException {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				classes.addAll(buscarClasesEnDirectorio(file, packageName + "." + file.getName()));
	...
	public void grabarArchivodeHelper(String paqueteCodigoGenerado, StringBuilder contenido, String path){
		String folderCodigoGenerado = paqueteCodigoGenerado.replace(".", "\\");
		String filename = path+folderCodigoGenerado+"/"+nombreClaseHelperVoEntity+".java";
		
		contenido.insert(0, importsClaseHelperVoEntity.toString());
		
		contenido.insert(0, NL+"/* En caso de cambiar el modelo de datos, esta clase debe ser modificada o volver a generar. */"+NL+NL);
		contenido.insert(0, NL+"* FECHA CREACION: "+new Date().toString()+" */"+NL);
